...
public:
    ...
    void Run();
    void Recv(int nums);
    //void add_cache_server(int port);  //添加cache_server的接口
...
void Server::Recv(int nums)
{
    ...
    {
        if(FD_ISSET(fd, &working_set)) //如果fd在woring_set中，则进行接收数据
        {
            int n;  // 标记当前连接是否断开了

            PACKET_HEAD head;
            n = recv(fd, &head, sizeof(head), 0);   // 先接受包头

            if(head.type == HEART)
            {
                mmap[fd].second = 0;        // 每次收到心跳包，count置0
                cout << "Received heart-beat from cache_server.\n";
            }
            else
            {
                // 数据包，通过head.length确认数据包长度
                cout<<"receive msg from cache_server"<<endl;
            }
...
{
    //thread th;     // 创建心跳检测线程
    //th = thread(heart_handler, (void*)this);  //创建线程，将this传进去
    //th.join();
    pthread_t id;
