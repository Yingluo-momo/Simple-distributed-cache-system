...
void taskconnect_class::sendout_data(){
    ...
    for(int j=0; j<(*ipport_and_key)[addr].size(); j++){
        ...
        Write(cachfd, (const char *)buf.data(), buf.length()+1);
        //Sleep_ms(100);
    }
    Close(cachfd);
}
...
void Heartbeat::heartbeat_connect(bool flag){
    ...
    do{
        ...
        auto info_con = Socket_connect(true, (struct sockaddr *)&serv_addr, sizeof(serv_addr));
        cfd = info_con.cfd;
        n = info_con.n;
        flag = true;
...
void Heartbeat::reflesh_handler(){
    ...
    if(!(sendout_bk->empty())){
        signal(SIGPIPE, SIG_IGN);
        int count=0;
        pthread_t takeout_allval[sendout_bk->size()];
        ...
        for(auto i = sendout_bk->begin(); i!=sendout_bk->end(); i++){
            if(curen_addr==i->first){
                for(int j=0; j<(*sendout_bk)[curen_addr].size(); j++){
                    ...
                    std::string temp_val = LC_BK->get(temp_key);
        ...
        Sleep_ms(200);
        for(int i=0; i<sendout_bk->size(); i++){
            if(flag==i)
                continue;
            pthread_join(takeout_allval[i], nullptr);
        }
